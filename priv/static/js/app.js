...
;require.register("js/multiple.js", function(exports, require, module) {
"use strict";
...
var msg = document.getElementById('sendtweet');
var subcrib = document.getElementById('subscribe');
//let tweets = document.getElementById('search'); 
//let hashsearch = document.getElementById('search_hashtag'); 

var logtrigger = document.getElementById('login_user');
var nametrigger = document.getElementById('register_userid');
var msgtrigger = document.getElementById('sendtweet_id');
var subcribtrigger = document.getElementById('subcribe_button');
//let tweetstrigger = document.getElementById('search_user_tweets'); 
//let hashsearchtrigger = document.getElementById('mentions');
var outtrigger = document.getElementById('log_out');
var cleartrigger = document.getElementById('clear_screen');

logtrigger.addEventListener("click", login_use);
nametrigger.addEventListener("click", registeratn);
subcribtrigger.addEventListener("click", subs);
msgtrigger.addEventListener("click", sent);
//tweetstrigger.addEventListener("click", search_tweet);
//hashsearchtrigger.addEventListener("click", searching_mentions);
outtrigger.addEventListener("click", outofhere);
cleartrigger.addEventListener("click", clear);
function clear() {
...
function login_use() {
  ...
  msgitem.innerText = log.value + " logged in at [" + Date() + "]";
  msgContainer.appendChild(msgitem);
}
function login(log) {
  channel.push("loggin", log).receive("loggedIn", function (resp) {
    return console.log("loggedIn", resp);
  });
}
//new client register
function registeratn() {
  register(name.value);
  var msgitem = document.createElement("li");
  msgitem.innerText = name.value + " registered in at [" + Date() + "]";
  msgContainer.appendChild(msgitem);
}

function register(name) {
  channel.push("register", name).receive("registered", function (resp) {
    return console.log("registered", resp);
  });
}

function subs() {
  var val = subcrib.value;
  subscribe(val, [name.value]);
  var msgitem = document.createElement("li");
  msgitem.innerText = name.value + " is subscribed to " + val + " on [" + Date() + "]";
  msgContainer.appendChild(msgitem);
  subscribe.value = "";
}

function subscribe(val, subslist) {
  channel.push("subscribe", { name: val, usersubs: subslist }).receive("subscribed", function (resp) {
    return console.log("subscribed", val);
  });
  console.log({ username: val, usersToSub: subslist });
}

function sent() {
  tweeting(msg.value, name.value);
  var msgitem = document.createElement("li");
  msgitem.innerText = name.value + " tweeted: " + msg.value + " at [" + Date() + "]";
  msgContainer.appendChild(msgitem);
  sendtweet.value = "";
}
function tweeting(text, name) {
  channel.push("tweet_now", { tweets: text,
    name: name });
  console.log({ tweets: text,
    name: name });
}

channel.on("tweetsub", function (message) {
  ...
  msgitem.innerText = "Tweeted: [" + Date() + "] " + message.tweets;
  messageButton.innerText = "RETWEET";
  messageButton.style.display = "inline";
  messageButton.addEventListener('click', function () {
    channel.push("retweet", { username: username.value, tweetText: message.tweetText });
  });
  // messageButton.style.float ="right"
  console.log(msgitem.innerText);
  msgContainer.appendChild(msgDiv);
});

/*function search_tweet(){
     console.log({name: name.value, time: `${Date()}`})
     channel.push("search", {name: name.value})
}
channel.on("search_result", message => {
    let msgitem = document.createElement("li");
    msgitem.innerText = `search result: [${Date()}] ${message.searched_tweet}`
    msgContainer.appendChild(msgitem)
  })

function searching_mentions(){
  
    channel.push("mentions", {name: name.value})
}
channel.on("mentions", message => {
    console.log(message.searched_tweet)
    let msgitem = document.createElement("li");
    msgitem.innerText = `search mentions: [${Date()}] ${message.searched_tweet}`
    msgContainer.appendChild(messageItem)
  })*/

function outofhere() {
  channel.push("logout", name.value).receive("loggedOut", function (resp) {
    return console.log("loggedOut", resp);
  });
  var msgitem = document.createElement("li");
  msgitem.innerText = name.value + " logged out at [" + Date() + "]";
  msgContainer.appendChild(msgitem);
}
